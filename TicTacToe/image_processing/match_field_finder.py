...
class MatchFieldFinder:
    ...
    def get_match_filed_base_fields(self, image, img_cnt):
        ...
        self.__img_optimizer.save_images_for_debug(
            gray, "gray", self.__cv_basic_conf.debug
        )

        gray = ImageOptimizer.transform_image_to_black_white(gray, 120)

        self.__img_optimizer.save_images_for_debug(
            gray, "gray_binary", self.__cv_basic_conf.debug
        )
        ...
        self.__img_optimizer.save_images_for_debug(
            edged, "canny", self.__cv_basic_conf.debug
        )
