...
def _get_image_link_target_from_file(
    dir_path, image_file, force_no_processing=False
):
    """
...
def _get_thumbnail_image_from_file(dir_path, image_file):
    ...
    try:
        ...
        print(
            "WARNING: IOError when thumbnailing %s/%s: %s"
...
def _run_server():
    ...
    print(
        "Your images are at http://127.0.0.1:%d/%s" % (port, INDEX_FILE_NAME)
    )
...
def serve_dir(dir_path):
    ...
    if PIL_ENABLED:
        ...
        # background
        print(
            "Performing PIL-enchanced optimised index file generation in background"
        )
