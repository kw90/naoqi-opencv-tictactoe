...
class UserInputCancelledByVoice(Exception):
    pass


def signal_handler(signum, frame):
    raise UserInputCancelledByVoice("voice input from user")
...
class GameHandler(object):
    def __init__(self, session, ip, motion, leds):
        ...
        self.__showScreen = ShowScreen(self.__session)
        self.__showScreen.show_screen("")
        self.__game = TicTacToe(
            self.__session, self.__speech, self.__showScreen
        )
        self.__moveReader = MoveReader(self.__session, self.__game)
        self.__speechDetection = SpeechDetection(
            self.__session, ["Let's Play", "Stop"], self.speech_callback
        )
        ...
        while self.__program_running:
            while self.__waiting_game_start:
                try:
                    ...
                    if input == "":
                        print("Program started with the keyboard.")
                except UserInputCancelledByVoice as ex:
                    print("Program started using voice.")
    ...
    def look_at_player_and_say(self, text, warning_led):
        ...
        angles = [78.0 * almath.TO_RAD, -30.0 * almath.TO_RAD]
        times = [1.0, 1.0]
        is_absolute = True
